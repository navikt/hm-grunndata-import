...
class SupplierAllowRule(rolesFinder: RolesFinder,
    ...
    companion object {
        private val LOG = LoggerFactory.getLogger(SupplierAllowRule::class.java)
    }
    ...
    override fun check(request: HttpRequest<*>, routeMatch: RouteMatch<*>?, authentication: Authentication?): Publisher<SecurityRuleResult> {
        if (routeMatch is MethodBasedRouteMatch<*, *> && authentication!=null ) {
            if (routeMatch.hasAnnotation(SupplierAllowed::class.java)) {
                val values = routeMatch.getValue(SupplierAllowed::class.java, Array<String>::class.java).get().toMutableList()
